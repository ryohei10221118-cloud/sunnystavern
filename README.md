...
> **不要照著上表猜**。每一條都要先量過才知道是不是你的狀況——
> 例如區域已經在東京、CPU 用量貼著 0%，那前面幾條就直接排除了。
> **[docs/diagnose.md](docs/diagnose.md)** 有一段可以貼進瀏覽器 Console 的程式碼，
> 會直接吐出 HTTP 協定、TTFB、請求數、最慢的資源和 API，一次把範圍縮到一條。
...
│   └── update.sh               備份 + 更新到最新版
├── docs/
│   ├── diagnose.md             量測「打開很慢」到底慢在哪
│   ├── lazy-monaco-extension.md 改良版 ST-Prompt-Template 的安裝與說明
