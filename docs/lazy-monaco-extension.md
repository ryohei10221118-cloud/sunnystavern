...
擴充載入的資源總量由 8.60MB 降為 0.69MB（**減少 92%**）。

本機乾淨環境（SillyTavern 1.18.0，localhost）：
...
雲端實機（Zeabur / Tokyo，掛載網路磁碟）：

| | 本體完成 | 擴充完成 | 落後本體 |
|---|---|---|---|
| 原版 | 2977ms | 26948ms | **+22.5 秒** |
| 改良版（重啟後第一次載入） | 21463ms | 26965ms | +5.5 秒 |
| 改良版（伺服器已熱） | 6152ms | 8721ms | **+2.57 秒** |

### 剩下的 2.57 秒是什麼

逐檔拆解顯示，那段時間完全是 `dist/index.js` 這一個檔案的傳輸：

```
5828ms → 8721ms   2894ms   627KB   dist/index.js
```

而且與原版的關係是**線性**的：檔案大 7.7 倍，時間就長 8.3 倍。
換句話說，原本那 22.5 秒從頭到尾就是「這個檔案傳完所需的時間」，
不是 Monaco 在解析、也不是初始化邏輯卡住。排在後面的擴充（例如 `regex`）
只是在等下載佇列。

單獨重抓同一個檔案時，速度與本體檔案相同（20~36 Mbps），沒有額外懲罰——
但那是作業系統快取命中後的熱讀取，無法用來否證「重啟後首次冷讀取較慢」。
與 `/api/settings/get` 的 801ms（冷）對比 137ms 以下（熱）是同一個模式。

**實務結論**：這 2.9 秒只發生在重啟後的第一次載入，之後走瀏覽器快取約 1 秒。
